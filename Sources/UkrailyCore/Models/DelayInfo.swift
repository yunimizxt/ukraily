...
enum DelayCategory: Int, Sendable {
    case onTime    // 0 min
    case minor     // 1–4 min
    ...
    static func from(minutes: Int) -> DelayCategory {
        switch minutes {
...
struct DelayInfo: Sendable {
    let minutes: Int
    let reason: String?
    let category: DelayCategory

    init(minutes: Int, reason: String? = nil) {
        self.minutes = minutes
        self.reason = reason
        self.category = DelayCategory.from(minutes: minutes)
    }

    static let none = DelayInfo(minutes: 0)
}
